...
def tokenizer_single(input_string):
    return input_string.split(" ")

def load_tokenizers():

    spacy_de = tokenizer_single
    spacy_en = tokenizer_single
...
def load_trained_model():
    config = {
        ...
        "max_padding": 72,
        "warmup": 3000,
        "file_prefix": "checkpoint/iwslt14_model_",
    }
    ...
    model = make_model(len(vocab_src), len(vocab_tgt), N=6)
    model.load_state_dict(torch.load("checkpoint/iwslt14_model_final.pt"))
    return model
