...
def check_outputs(
    ...
    n_examples = len(valid_dataloader)
    ...
    for idx in range(n_examples):
        ...
        greedy_decode(model, rb.src, rb.src_mask, 64, 0)

        batch_size = 32
        model_out = greedy_decode(model, rb.src, rb.src_mask, 72, 0)
        for i in range(batch_size):
            src_tokens = [
                vocab_src.get_itos()[x] for x in rb.src[i] if int(x) != pad_idx
            ]
            tgt_tokens = [
                vocab_tgt.get_itos()[x] for x in rb.tgt[i] if int(x) != pad_idx
            ]
            ...
            model_txt = (
                " ".join(
                    [vocab_tgt.get_itos()[x] for x in model_out[i] if x != pad_idx]
            ...
            print(output_target)
            print("LIST:")
            output_list = [vocab_tgt.get_itos()[x] for x in model_out[i] if int(x) != pad_idx]

            if "</s>" not in output_list:
                print("NOPE")
                not_working_counter = not_working_counter + 1
                continue
...
def run_model_example(n_examples=5):
    ...
    _, valid_dataloader = create_dataloaders(
        torch.device("cuda"),#available_gpus[0],
        vocab_src,
        vocab_tgt,
        spacy_de,
        spacy_en,
...
def greedy_decode(model, src, src_mask, max_len, start_symbol):
    ...
    for i in range(max_len - 1):
        out = model.decode(
            memory, src_mask, ys, subsequent_mask(ys.size(1)).type_as(src.data).to(src.device)
        )
