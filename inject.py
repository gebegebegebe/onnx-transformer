...
def check_outputs(
    ...
    for idx in range(n_examples):
        ...
        greedy_decode(model, rb.src, rb.src_mask, 64, 0, module_component)[0]
        ...
        model_out = greedy_decode(model, rb.src, rb.src_mask, 72, 0, module_component)[0]
        model_txt = (
            " ".join(
...
def run_model_example(run_module_component="encoder", n_examples=5):
    ...
    example_data = check_outputs(
        valid_dataloader, model, vocab_src, vocab_tgt, n_examples=n_examples, module_component=module_component
    )
...
def load_trained_model(module_component):
    config = {
        "batch_size": 32,
        "distributed": False,
        "num_epochs": 8,
    ...
    if not exists(model_path):
        ...
        run_model_example(module_component)
