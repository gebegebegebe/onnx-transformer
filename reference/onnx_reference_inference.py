...
def greedy_decode(model, src, src_mask, max_len, start_symbol):
    ...
    ort_sess_encoder = ort.InferenceSession('./onnx/fixed/encoder_fixed.onnx')
    memory = torch.from_numpy(ort_sess_encoder.run(None, {"global_in": src_float.detach().numpy(), "global_in_1": src_mask.detach().numpy()})[0])
